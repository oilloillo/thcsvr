...
--红魔-蕾米莉亚·斯卡雷特
function c22025.initial_effect(c)
	--synchro summon
	...
	e1:SetDescription(aux.Stringid(22025,0))
	e1:SetType(EFFECT_TYPE_QUICK_O)
	e1:SetCode(EVENT_FREE_CHAIN)
	e1:SetRange(LOCATION_MZONE)
	e1:SetHintTiming(0,0x1e0)
	e1:SetCondition(c22025.condition)
	e1:SetCost(c22025.cost)
	e1:SetTarget(c22025.target)
	e1:SetOperation(c22025.operation)
	c:RegisterEffect(e1)
	--Race
	...
	e4:SetDescription(aux.Stringid(22025,1))
	e4:SetCategory(CATEGORY_TODECK)
	e4:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e4:SetType(EFFECT_TYPE_TRIGGER_F+EFFECT_TYPE_SINGLE)
	e4:SetCode(EVENT_SPSUMMON_SUCCESS)
	e4:SetCondition(c22025.tdcon)
	e4:SetTarget(c22025.tdtg)
	e4:SetOperation(c22025.tdop)
	c:RegisterEffect(e4)
end
function c22025.costfilter(c)
	return (c:IsSetCard(0x813) or c:GetOriginalCode()==(22100) or c:GetOriginalCode()==(22117)) and c:IsAbleToRemoveAsCost()
end
function c22025.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c22025.costfilter,tp,LOCATION_GRAVE,0,2,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local g=Duel.SelectMatchingCard(tp,c22025.costfilter,tp,LOCATION_GRAVE,0,2,2,nil)
	Duel.Remove(g,POS_FACEUP,REASON_COST)
end
function c22025.condition(e,tp,eg,ep,ev,re,r,rp)
	local seq=e:GetHandler():GetSequence()
	return e:GetHandler():IsFaceup()
		and Duel.GetFieldCard(1-tp,LOCATION_MZONE,4-seq)
end
function c22025.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
...
function c22025.operation(e,tp,eg,ep,ev,re,r,rp)
...
function c22025.tdcon(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():GetSummonType()==SUMMON_TYPE_SYNCHRO
end
function c22025.tdtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
...
function c22025.tdop(e,tp,eg,ep,ev,re,r,rp)
