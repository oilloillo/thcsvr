...
function c23204.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsCanRemoveCounter(tp,1,1,0x208a,6,REASON_COST) end
	Duel.RemoveCounter(tp,1,1,0x208a,6,REASON_COST)
end
...
function c23204.cfilter(c)
	return c:IsSetCard(0x497) and c:IsAbleToDeckAsCost() and c:GetCounter(0x208a)>0
end
function c23204.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TODECK)
		local g=Duel.SelectMatchingCard(tp,c23204.cfilter,tp,LOCATION_MZONE,0,1,1,nil)
			e:SetLabel(g:GetFirst():GetCounter(0x208a))
...
function c23204.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc then
		...
		if e:GetLabel()>0 then
			tc:AddCounter(0x208a,e:GetLabel())
		end
	end
end
