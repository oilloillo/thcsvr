...
function c28011.initial_effect(c)
	...
	e3:SetCountLimit(1)
	e3:SetTarget(c28011.target)
	e3:SetOperation(c28011.operation)
	c:RegisterEffect(e3)
	--special summon
	local e4=Effect.CreateEffect(c)
	e4:SetDescription(aux.Stringid(28011,2))
	e4:SetCategory(CATEGORY_TOGRAVE+CATEGORY_SPECIAL_SUMMON)
	e4:SetProperty(EFFECT_FLAG_DAMAGE_STEP+EFFECT_FLAG_CHAIN_UNIQUE)
	e4:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_TRIGGER_F)
	e4:SetCode(EVENT_DESTROY)
	e4:SetRange(LOCATION_REMOVED)
	e4:SetCondition(c28011.scon)
	e4:SetCost(c28011.cost)
	e4:SetTarget(c28011.stg)
	e4:SetOperation(c28011.sop)
	c:RegisterEffect(e4)
end
function c28011.rfilter(c)
	return c:IsSetCard(0x211) and c:IsAbleToRemoveAsCost()
end
function c28011.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c28011.rfilter,tp,LOCATION_GRAVE,0,2,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local g=Duel.SelectMatchingCard(tp,c28011.rfilter,tp,LOCATION_GRAVE,0,2,2,nil)
	Duel.Remove(g,POS_FACEUP,REASON_COST)
end
function c28011.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c28011.sop(e,tp,eg,ep,ev,re,r,rp)
	local g=Duel.GetMatchingGroup(aux.TRUE,tp,0xf,0xf,nil)
