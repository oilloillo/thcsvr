...
function c21056.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	local mg=Duel.GetMatchingGroup(c21056.filter,tp,LOCATION_GRAVE,0,nil,e,tp)
	local exg=Duel.GetMatchingGroup(c21056.xyzfilter,tp,LOCATION_EXTRA,0,nil,mg)
...
function c21056.filter2(c,e,tp)
	return c:IsSetCard(0x208) and c:IsRelateToEffect(e) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function c21056.activate(e,tp,eg,ep,ev,re,r,rp)
	if Duel.IsPlayerAffectedByEffect(tp,59822133) then return end
	if Duel.GetLocationCount(tp,LOCATION_MZONE)<2 then return end
	local g=Duel.GetChainInfo(0,CHAININFO_TARGET_CARDS):Filter(c21056.filter2,nil,e,tp)
	if g:GetCount()<2 then return end
	Duel.SpecialSummon(g,0,tp,tp,false,false,POS_FACEUP)
	Duel.BreakEffect()
	local xyzg=Duel.GetMatchingGroup(c21056.xyzfilter,tp,LOCATION_EXTRA,0,nil,g)
