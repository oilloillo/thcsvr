...
function c22052.initial_effect(c)
	...
	e2:SetCondition(c22052.scon)
	e2:SetTarget(c22052.stg)
	e2:SetOperation(c22052.sop)
	c:RegisterEffect(e2)
	local e3=e2:Clone()
	...
	e4:SetCondition(c22052.condition)
	e4:SetCost(c22052.cost)
	e4:SetTarget(c22052.target)
	e4:SetOperation(c22052.operation)
	c:RegisterEffect(e4)
	if not c22052.global_check then
		c22052.global_check=true
		local ge1=Effect.CreateEffect(c)
		ge1:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_CONTINUOUS)
		ge1:SetCode(EVENT_SPSUMMON_SUCCESS)
		ge1:SetOperation(c22052.checkop)
...
function c22052.checkop(e,tp,eg,ep,ev,re,r,rp)
...
function c22052.scon(e,tp,eg,ep,ev,re,r,rp)
	return ep~=tp
end
function c22052.stg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return e:GetHandler():IsCanBeSpecialSummoned(e,0,tp,false,false) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,e:GetHandler(),1,0,0)
end
function c22052.sop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and c:IsRelateToEffect(e) then
		Duel.SpecialSummon(c,0,tp,tp,false,false,POS_FACEUP)
	end
end
function c22052.condition(e,tp,eg,ep,ev,re,r,rp)
...
function c22052.operation(e,tp,eg,ep,ev,re,r,rp)
