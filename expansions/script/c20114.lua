...
function c20114.desreptg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return not c:IsReason(REASON_REPLACE)
		and c:IsCanRemoveCounter(tp,0x208b,1,REASON_EFFECT) end
	if Duel.SelectYesNo(tp,aux.Stringid(20114,0)) then
		c:RemoveCounter(tp,0x208b,1,REASON_EFFECT)
		return true
	else return false end
end
function c20114.descost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return e:GetHandler():IsCanRemoveCounter(tp,0x208b,3,REASON_COST) end
	e:GetHandler():RemoveCounter(tp,0x208b,3,REASON_COST)
end
