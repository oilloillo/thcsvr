...
function c22056.initial_effect(c)
	...
	e3:SetCondition(c22056.damcon)
	e3:SetTarget(c22056.damtg)
	e3:SetOperation(c22056.damop)
	c:RegisterEffect(e3)
	--special summon
...
function c22056.damcon(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local bc=c:GetBattleTarget()
	return c:IsRelateToBattle() and bc:IsLocation(LOCATION_GRAVE) and bc:IsReason(REASON_BATTLE)
end
function c22056.damtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	Duel.SetTargetPlayer(1-tp)
	Duel.SetTargetParam(1200)
	Duel.SetOperationInfo(0,CATEGORY_DAMAGE,nil,0,1-tp,1200)
end
function c22056.damop(e,tp,eg,ep,ev,re,r,rp)
	local p,d=Duel.GetChainInfo(0,CHAININFO_TARGET_PLAYER,CHAININFO_TARGET_PARAM)
	Duel.Damage(p,d,REASON_EFFECT)
end
function c22056.spfilter(c,tp)
	return c:IsSetCard(0x813) and c:GetPreviousControler()==tp
end
function c22056.spcon(e,tp,eg,ep,ev,re,r,rp)
	return eg:IsExists(c22056.spfilter,1,nil,tp)
end
function c22056.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and e:GetHandler():IsCanBeSpecialSummoned(e,0,tp,false,false) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,e:GetHandler(),1,0,0)
end
function c22056.spop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
